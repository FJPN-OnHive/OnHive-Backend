...
namespace OnHive.Students.Services
{
    public class StudentsService : IStudentsService
    {
        ...
        public async Task<StudentDto?> InternalEnroll(EnrollmentMessage enrollment)
        {
            ...
            await activitiesService.RegisterActivity(mapper.Map<StudentDto>(currentStudent), mapper.Map<StudentCourseDto>(course), null, StudentEventTypes.Enroll, "Matr�cula Realizada", $"Matr�cula Realizada {course.Id}");
            return GetStudentResume(currentStudent);
        }
        ...
        public async Task<StudentDto?> Enroll(EnrollmentMessage enrollment, LoggedUserDto loggedUser)
        {
            ...
            currentStudent = await studentsRepository.SaveAsync(currentStudent, enrollment.UserId);
            RegisterEventEnrollment(EventKeys.EnrollmentCreated, "Enrollment created", user, currentStudent, course, product);
            await activitiesService.RegisterActivity(mapper.Map<StudentDto>(currentStudent), mapper.Map<StudentCourseDto>(currentStudent.Courses.Find(c => c.Id == course.Id)), null, StudentEventTypes.Enroll, "Matr�cula Realizada", $"Matr�cula Realizada {course.Id}");
            return GetStudentResume(currentStudent);
        }
        ...
        public async Task<StudentDto?> FreeEnroll(EnrollmentMessage enrollment, LoggedUserDto loggedUser)
        {
            ...
            await activitiesService.RegisterActivity(mapper.Map<StudentDto>(currentStudent), mapper.Map<StudentCourseDto>(estudentCourse), null, StudentEventTypes.Enroll, "Matr�cula Gratuita Realizada", $"Matr�cula Gratuita Realizada {course.Id}");
            return GetStudentResume(currentStudent);
        }
        ...
        public async Task<StudentDto?> UnEnrollment(string studentId, string courseId, LoggedUserDto loggedUser)
        {
            ...
            await activitiesService.RegisterActivity(mapper.Map<StudentDto>(currentStudent), mapper.Map<StudentCourseDto>(currentStudent.Courses.Find(c => c.Id == courseId)), null, StudentEventTypes.UnEnroll, "Matr�cula Cancelada", $"Matr�cula Cancelada {courseId} Por {loggedUser.User.MainEmail} - {loggedUser.User.Id}");
        ...
        private async Task<StudentProgressResponseDto> ProcessSurveySubmit(StudentLesson lesson, StudentExamSubmitDto examSubmit, StudentCourseDto studentCourse, LoggedUserDto loggedUser)
        {
            ...
            await activitiesService.RegisterActivity(loggedUser, studentCourse, mapper.Map<StudentLessonsDto>(lesson), StudentEventTypes.SurveyResponse, "Avalia��o respondida", $"Avalia��o respondida {lesson.Id} do curso {course.Id}");
            return result;
        }
